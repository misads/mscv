...
def print_network(net: nn.Module, print_size=False, logger=None, image_size=None):
    ...
    Example
        ...
        >>> from mscv import print_network
        >>>
    ...
    for name, param in net.named_parameters():
        ...
        if len(size) > 1:
            if print_size:
                output = f'{name} {size[1:2]+size[:1]+size[2:]} {params_to_string(param.numel())}'
            else:
                output = f'{name} {size[1:2]+size[:1]+size[2:]}'
            if logger is None:
                print(output)
            else:
                logger.info(output)
    ...
    if image_size is not None:
        output = 'Flops: %s' % flops_to_string(get_model_flops(net, image_size))
        if logger is None:
            print(output)
        else:
            logger.info(output)
...
def get_model_flops(model, input_res, input_constructor=None):
    """
    Args:
        model(nn.Module): network model.
        input_res(tuple): image size (c, h, w).
        input_constructor(func): input constructor.
